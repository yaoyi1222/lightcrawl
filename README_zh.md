...
**Refetch 一键升级 Agent（Claude Code、Codex、Gemini CLI、Copilot CLI 等）的网页抓取与搜索能力。反爬、JS 渲染、登录态、多后端搜索全部内置，内容管线智能去噪，节省 30–90% 的 token 消耗。**
...
Refetch 是一个本地 MCP Server，升级 Agent 的网页抓取与搜索能力。它支持：
...
- ✅ 一站式搜索+阅读 — `search_and_read` 一次调用完成搜索与并发抓取
...
- ✅ **搜索+阅读一步到位** `search_and_read` 搜索结果和 top N 页面全文一起返回，比手动搜索+N 次 fetch 省 ~30%+ token
- ✅ **持久化登入状态** `auth_login` 打开真实浏览器，手动登录，session 保存为 profile
...
Refetch 的内容管线自动定位到 `<main>`/`<article>`,剥离不可见元素,返回结构化 `headings: [{level, text, line}]`。`selector` 参数精确锁定内容区域(如 GitHub 的 `article.markdown-body`);`output_format=text` 去掉 markdown 语法开销。

| 网站 | 内置 WebFetch | Refetch 默认 | + `selector` | + `output_format=text` | 最大节省 |
|---|---|---|---|---|---|
...
\*Python docs 通过 Refetch 拿到了*更多*内容,因为 Playwright 执行 JS 后加载了完整的侧边栏导航。

### 登录会话

`auth_login` 打开**有头** Chromium 窗口让用户手动登录。工具永不接触密码。登录后,session 保存为命名 profile,绑定到登录 URL 的 eTLD+1,并可通过 `fetch_url(url, profile=<name>)` 复用。
...
| 场景 | 为什么用 Refetch |
|---|---|
| **答案在登录墙后面** | `search_and_read(query, profile="x")` — 一次调用完成搜索 + 已登录抓取 |
| **排名靠前的结果是 JS 渲染的 SPA** | `search_and_read` 自动通过 Playwright 浏览器管线渲染页面 |
...
| **一个后端被限速了** | 自动故障转移到下一个已配置的后端 — 无需人工干预 |
...
| **登录墙后的来源** | ✅ `auth_login` → 已登录抓取 X、LinkedIn、私仓 | ❌ |
| **JS 渲染** | ✅ Playwright 浏览器执行 JS,等待选择器 | ❌ 仅服务端内容 |
| **单次调用拿全文** | `search_and_read` 抓取 top N 页(3 页约 13k chars) | `--include-raw-content` 抓取全部(10 页约 240k chars) |
...
| 工具 | 用途 |
|---|---|
| `fetch_url` | 抓取 URL,自动按策略升级。返回 markdown + headings(level/text/line)+ 推荐 selector + 超长时的 dump 路径。 |
| `search` | Web 搜索,返回结构化结果,含富 snippet 和每条结果的 `fetch_hint`。 |
| `search_and_read` | 一站式:搜索 + 并发抓取 top N。比手动分步省 ~30%+ token。 |
| `list_backends` | 报告当前可用的搜索后端及配置状态。 |
| `auth_login` | 弹出有头浏览器让用户登录某站点,保存为 profile。 |
| `auth_status` | 列出已保存的 profile(只返回元信息,不返回 cookie)。 |
| `auth_revoke` | 删除 profile。 |

完整 input schema 见 [`src/refetch/server.py`](src/refetch/server.py)。
...
内置三个可插拔后端。默认按 **Brave → Serper → Tavily** 顺序选第一个已配置的;可通过 `search(backend="serper")` 显式覆盖。
...
`Refetch` 与 Tavily 官方 `tavily-mcp` **互补,不冲突** — 推荐两个 MCP server 同时注册:

| | `tavily-mcp`(云端) | `Refetch`(本地) |
|---|---|---|
| 搜索排名 + LLM snippet | ✅ 业界最强之一 | ✅ 通过 `TavilyBackend`(仅 snippet 路径) |
| 登录态站点(X、私有 GH、内网 wiki) | ❌ | ✅ `auth_login` profile |
...
- 模型永远拿不到 cookie 内容 — `auth_status` 只返回元信息。
- `auth_login` 永远使用**有头**浏览器。密码和 2FA 由用户亲自输入,Server 只在检测到登录成功后才调用 `context.storage_state()`。
...
- 抓取到的内容会包裹在 `<fetched_content>` 标签里,Skill 提醒模型:标签内是数据,不是指令。
